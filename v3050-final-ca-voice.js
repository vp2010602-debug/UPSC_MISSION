...
(function(){
  ...
  window.sendCAToSaarthiV305=async id=>{const t=getTopic(id);if(!t)return;await saveCollection('saarthiTasksV303',{title:`Revise CA: ${t.title}`,subject:t.subject,type:'Current Affairs',dueDate:new Date(Date.now()+86400000).toISOString().slice(0,10),frequency:'One-time',priority:t.importance,details:t.revisionCapsule.join('; '),status:'assigned',source:'Daily CA Tracker'});toast('Added to Saarthi weekly targets.','success')};
